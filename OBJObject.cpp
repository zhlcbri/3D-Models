...
void OBJObject::parse(const char *filepath)
{
    ...
    ifs.open(filepath);
    if (!ifs.is_open()) {
        cerr << "error loading the file: " << filepath << endl;
        exit(-1);
...
void OBJObject::draw(GLuint shaderProgram)
{
    ...
    glUniform3f(glGetUniformLocation(shaderProgram, "light.light_dir"), -0.3f, -0.5f, -1.0f);
    glUniform3f(glGetUniformLocation(shaderProgram, "light.light_color"), 1.0f, 1.0f, 1.0f);
    
    // Send Material properties to shader program
    if (Window::obj_mode == 0) { // bunny
    ...
    //cout << "obj_mode: " << Window::obj_mode << endl;
    else if (Window::obj_mode == 1) { // bear
    ...
    // Send Light info to shader program
    if (Window::light_mode == 2) { // point light
        //vec3(1.2f, 1.0f, 2.0f)
        glUniform3f(glGetUniformLocation(shaderProgram, "light.light_pos"), 1.2f, 1.0f, 2.0f); // for now
    }
